...
if __name__ == "__main__":
    ...
    # Update num_relation in config for both retrievers
    # Handle nested base_layer structure or flat structure
    if hasattr(cfg.context_retriever, 'kg_encoder'):
        if hasattr(cfg.context_retriever.kg_encoder, 'base_layer'):
            cfg.context_retriever.kg_encoder.base_layer.num_relation = int(dataset.kgdata.num_relation)
        else:
            cfg.context_retriever.kg_encoder.num_relation = int(dataset.kgdata.num_relation)
    
    if hasattr(cfg.score_retriever, 'kg_encoder'):
        if hasattr(cfg.score_retriever.kg_encoder, 'base_layer'):
            cfg.score_retriever.kg_encoder.base_layer.num_relation = int(dataset.kgdata.num_relation)
        else:
            cfg.score_retriever.kg_encoder.num_relation = int(dataset.kgdata.num_relation)
